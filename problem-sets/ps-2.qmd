...
-   `pct65plus` : The proportion of voters at least 65 years in a given county.
...
b)  Create a visualization of the relationship between `inperson_pct` and `pctTrump_2016`. Draw the least squares regression line along the cloud of points. Based on the visualization, how would you describe the relationship?
...
e)  Use the linear model to predict the proportion of in-person votes in a county where 75% of the votes were cast for Trump in 2016.
...
In this problem, we will try to answer the question of whether counties with higher proportions of older voters (65 year plus) tend to lean republican. You will use the variables `pct65plus` that provides the proportion of voters in every county that were 65 or older and **`pctTrump_2016`**, that provides the proportion of votes cast for Trump in 2016 in a given county.

a)  Create a new data set (name it `nc_election_b`) by adding a binary variable called `senior_voters` with levels `high` and `low`. A county is considered to have high rates of older voters if the proportion of voters 65 years or older was greater than or equal to 20% (0.2).

b)  Fit a linear regression model to predict the proportion of trump votes in 2016 (**`pctTrump_2016`**) based on the binary variable `senior_voters`. Write down the equation of the model.
...
The quarto template for this problem set is available under the downloads section of the course website. You may want to visit the FAQs page to learn how to upload the template to your posit cloud. Once you have completed the problem set, you should render the document to a pdf then submit ***both the pdf and the source file (quarto)*** to Canvas.
